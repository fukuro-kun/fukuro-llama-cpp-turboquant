...
// Per-layer on the respective layer device to ensure all ggml_cpy / ggml_concat
// operations are intra-backend (avoids cross-backend copy failures).
static void dg_ensure_pkv_store(const llama_model & m, int64_t P) {
    if (!m.pkv_k.empty() && m.pkv_cap >= P) {
        return;
    }
    for (auto buf : m.pkv_bufs) { ggml_backend_buffer_free(const_cast<ggml_backend_buffer_t>(buf)); }
    for (auto ctx : m.pkv_ctxs) { ggml_free(const_cast<ggml_context *>(ctx)); }
    const_cast<llama_model &>(m).pkv_bufs.clear();
    const_cast<llama_model &>(m).pkv_ctxs.clear();
    ...
    // Group layers by their backend buffer-type so each group's PKV tensors
    // live on the same backend as Kcur/Vcur for those layers.
    ...
    for (auto & [buft, layers] : buft_layers) {
        ...
        ggml_init_params ip = {
            /*.mem_size   =*/ ggml_tensor_overhead() * (2 * n + 2),
            /*.mem_buffer =*/ nullptr,
            /*.no_alloc   =*/ true,
        };
        ...
        for (int il : layers) {
            ...
            const_cast<llama_model &>(m).pkv_k[il] = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, hd, nkv, cap);
            const_cast<llama_model &>(m).pkv_v[il] = ggml_new_tensor_3d(ctx, GGML_TYPE_F32, hd, nkv, cap);
        ...
        const_cast<llama_model &>(m).pkv_ctxs.push_back(ctx);
        const_cast<llama_model &>(m).pkv_bufs.push_back(buf);
    }
