...
void llama_model_gemma4_assistant::load_arch_hparams(llama_model_loader & ml) {
    ...
    // Older GGUF files may not set n_layer_nextn; default to n_layer_all
...
void llama_model_gemma4_assistant::load_arch_tensors(llama_model_loader &) {
    ...
    if (hparams.n_embd_out() == n_embd) {
        // Older GGUF files may not set embedding_length_out; this is acceptable
        // for backward compatibility but may cause issues with newer features.
        LLAMA_LOG_WARN("Gemma 4 assistant: embedding_length_out not set, using n_embd as fallback\n");
    }
