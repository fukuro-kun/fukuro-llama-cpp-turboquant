...
llama_model::~llama_model() {
    ...
    // DiffusionGmma cleanup (monolithic port)
...
// =============================================================================
// DiffusionGmma Dummy Memory
// DiffusionGmma does not use a KV cache (it uses a prompt-KV store instead),
// but llama_decode requires a memory object to run the normal decode path.
// This dummy memory forwards batches without any caching.
// =============================================================================

class llama_memory_diffusion_context : public llama_memory_context_i {
public:
    explicit llama_memory_diffusion_context(std::vector<llama_ubatch> ubatches)
        : ubatches(std::move(ubatches)), i_cur(0), status(LLAMA_MEMORY_STATUS_SUCCESS) {}

    explicit llama_memory_diffusion_context(llama_memory_status status)
        : i_cur(0), status(status) {}

    bool next() override {
        i_cur++;
        return i_cur < ubatches.size();
    }
...
class llama_memory_diffusion : public llama_memory_i {
public:
    llama_memory_context_ptr init_batch(
        ...
        if (ubatches.empty()) {
            return std::make_unique<llama_memory_diffusion_context>(LLAMA_MEMORY_STATUS_FAILED_PREPARE);
        }

        return std::make_unique<llama_memory_diffusion_context>(std::move(ubatches));
    }

    llama_memory_context_ptr init_full() override {
        // Full cache simulation: create a single-token ubatch (worst-case)
        return std::make_unique<llama_memory_diffusion_context>(LLAMA_MEMORY_STATUS_SUCCESS);
    }

    llama_memory_context_ptr init_update(llama_context * lctx, bool optimize) override {
        GGML_UNUSED(lctx);
        GGML_UNUSED(optimize);
        return std::make_unique<llama_memory_diffusion_context>(LLAMA_MEMORY_STATUS_NO_UPDATE);
    }

    bool get_can_shift() const override { return false; }

    void clear(bool data) override { GGML_UNUSED(data); }

    bool seq_rm  (llama_seq_id seq_id, llama_pos p0, llama_pos p1) override {
        GGML_UNUSED(seq_id); GGML_UNUSED(p0); GGML_UNUSED(p1);
        return true;
    }

    void seq_cp  (llama_seq_id seq_id_src, llama_seq_id seq_id_dst, llama_pos p0, llama_pos p1) override {
        GGML_UNUSED(seq_id_src); GGML_UNUSED(seq_id_dst); GGML_UNUSED(p0); GGML_UNUSED(p1);
    }

    void seq_keep(llama_seq_id seq_id) override { GGML_UNUSED(seq_id); }
    ...
    void state_write(llama_io_write_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) const override {
        GGML_UNUSED(io); GGML_UNUSED(seq_id); GGML_UNUSED(flags);
    }
