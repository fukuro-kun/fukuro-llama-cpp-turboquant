...
- **❌ #45 CUDA Concurrent Streams QKV — bereits im Fork, Regression mit CUDA Graphs** — Feature vollständig implementiert (upstream PR #16991, `GGML_CUDA_GRAPH_OPT=1` env var). Benchmark auf Uranus (RTX 4060 Ti, E4B Q4_K_XL): Mit CUDA Graphs tg2048 71.50→63.82 t/s (-10.7%, ±18.77 Varianz — Graph-Capture ineffizient durch interleaved Node-Order). Ohne CUDA Graphs tg512 72.24→72.99 (+1%, im Rauschen — QKV-Projektionen zu klein bei E4B). Feature nicht aktivierbar ohne CUDA-Graph-Regression. ROADMAP #45 auf ❌ gesetzt.
