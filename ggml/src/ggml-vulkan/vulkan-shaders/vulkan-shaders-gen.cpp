...
void process_shaders() {
    ...
    for (const bool& fp16 : {false, true}) {
        ...
        for (const bool& f16acc : {false, true}) {
            ...
            for (const auto& tname : type_names) {
                ...
                } else if (tname == "q4_0" || tname == "q4_1" || tname == "q5_0" || tname == "q5_1" || tname == "iq4_nl" || tname == "q8_0" || tname == "f32" || tname == "turbo3_0" || tname == "turbo4_0") {
                    std::string data_a_key = "DATA_A_" + to_uppercase(tname);
                    string_to_spv("flash_attn_f32_f16_" + tname, "flash_attn_cm1.comp",
...
#if defined(GGML_VULKAN_INTEGER_DOT_GLSLC_SUPPORT)
                    // MMQ path has no turbo3_0/turbo4_0 code path; skip.
                    if (tname != "f32" && tname != "turbo3_0" && tname != "turbo4_0") {
                        string_to_spv("flash_attn_f32_f16_" + tname, "flash_attn.comp",
    ...
    // turbo3_0/turbo4_0 copy-from-quant only; copy-to-quant (cpy_f32_turboN_0) omitted because the non-SET_ROWS quantize() path lacks the WHT transform
    ...
    // turbo4_0: SET_ROWS temporär deaktiviert — quantize() Shader-Helfer fehlt noch
    // (TURBO4_0 Lesen/dequant funktioniert, Schreiben/SET_ROWS folgt)
