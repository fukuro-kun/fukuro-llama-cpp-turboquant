#!/usr/bin/env bash
# Stoppt den Uranus 26B-A4B llama-server (1 Instanz, GPU 0).
# Rasch abbaubar — sauberes Shutdown.

set -euo pipefail

PIDFILE="/tmp/uranus-26b-gpu0.pid"
PORT="${PORT:-18080}"
...
if [[ -f "$PIDFILE" ]]; then
  ...
  if kill -0 "$PID" 2>/dev/null; then
    echo "Stoppe llama-server (PID $PID, Port $PORT)..."
    ...
    echo "llama-server (PID $PID) läuft nicht mehr."
  fi
  rm -f "$PIDFILE"
else
...
if [[ "$STOPPED" == "1" ]]; then
  ...
  echo "=== Kein laufender 26B-Server gefunden ==="
fi
