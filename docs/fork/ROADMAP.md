...
| 40 | ⏳ | MoE Load Balancing Expert Frequency | Styx, Hydra | 2-3 Wochen | nein (Konzept) | — | bessere Load-Balance bei MoE-Offloading — **Phase 1 implementiert (2026-07-13): Expert Frequency Tracking via llama_set_moe_freq_track()/llama_get_moe_freq() C API. build_moe_ffn erzeugt ggml_cont copy der selected_experts, als output markiert. llama-bench prints per-layer top-3/bottom-3 Report via LLAMA_MOE_FREQ_TRACK=1. Validiert auf Styx (26B QAT, pp2048+tg128): alle 30 Layer, 128 Experts/Layer, korrekte Counts. Heisse/kalte Experten identifizierbar. Phase 2: datengetriebene Expert-Platzierung (hot→GPU, cold→CPU).** |
