...
| 57 | ❌ | MMQ Stream-k Disable für Tensor-Split MoE | Uranus | 2-4h | PR #22170 (merged) | — | 30-50% P2P PP auf Dual-GPU MoE — **Implementiert+Benchmarked 2026-07-13: GGML_CUDA_DISABLE_MMQ_STREAM_K env var hinzugefügt. Benchmark auf Uranus (2x RTX 4060 Ti, E4B QAT Q4_K_XL, 4GB): pp512 -64% Regression mit stream-k OFF (layer split: 4778→1701, tensor split: 3962→1355). tg128 unbeeinflusst. E4B ist zu klein (7.46B) — stream-k decomposition HILFT bei der Lastverteilung. PR #22170 war für große MoE-Modelle (26B+) auf Dual 3090 gedacht wo stream-k fixup-buffer Race Conditions verursacht. Env var bleibt im Code für zukünftige große MoE-Modelle, aber Default bleibt OFF (stream-k ON).** |
