...
| 19 | 🔬 | Expected Attention KV Cache Compression | Alle | 4-6 Wochen | arXiv:2510.00636 | Phase 2c ✅ | ergänzt TurboQuant — **Phase 1 CPU Math ✅ (12 Tests, 135 Assertions). Phase 2a/b ✅: CLI-Parameter (`--ea-ratio`, `--ea-future`, `--ea-sink`, `--ea-local`, `--ea-no-covariance`, `--ea-no-vnorm`), `llama_context_params` + `llama_cparams` Integration. Phase 2c ✅: `ea_compress_stub()` in `llama_kv_cache` (validiert Parameter, kein echtes Pruning). Phase 2d ausstehend: Echtes Pruning mit `seq_rm()` + Attention-Mask-Update.** |
