...
| # | Status | Ansatz | Systeme | Aufwand | Existiert | Solo-Plan | Gain |
|---|--------|--------|---------|---------|-----------|-----------|------|
| 8 | ☐ | Mixed Precision KV Cache (Hot/Cold) | Alle | 2-3 Wochen | commit e889fbd | später | Reduziert KV-Größe bei erhaltener Qualität — **Eval 2026-07-13: Commit e889fbd nicht im Fork, keine Impl in ggml/src/ oder src/. Feature: recent tokens FP16, older auto-quantized (threshold=32, group_size=16). Implementierbar.** |
...
| 15 | ☐ | PipeShard: VRAM-Constrained MoE | Styx, Hydra, Mars | 4-6 Wochen | arXiv:2604.26334 | [M3](plans/SESSION_PLAN_pipeshard.md) | VRAM-optimierte MoE-Inference — **Eval 2026-07-13: Nicht im Fork, Paper ist llama.cpp-Fork (Code auf github.com/deepshnv/pipeshard-mlsys26-ae), architektur-kompatibel, 2-3 Wochen Aufwand. SESSION_PLAN_pipeshard.md existiert nicht.** |
...
| 34 | ☐ | UBBoost: Dynamische Ubatch-Größe | Styx, Hydra | 2-3 Wochen | Discussion #23262 | — | bis 2x PP bei VRAM-konstriktiven Setups — **Eval 2026-07-13: Nicht im Fork. RFC #23262 zeigt bis 2x PP. Fork hat ubatch infra (n_ubatch=512 default, -ub CLI flag). Ansatz: ubatch für prefill vergrößern, decode klein halten. ~1 Woche Aufwand.** |
...
| 37 | ☐ | LFRU Expert Caching | Styx, Hydra | 2-4 Wochen | vLLM commit 71ed1fc | — | +5.2% speedup, 15-20% hit rate — **Eval 2026-07-13: Nicht im Fork. thecodacus Memory Pinning ist Foundation (async prefetch + pinned memory, +72-106% pp). LFRU eviction policy wäre inkrementell (~1000-1500 LOC). Für Styx (8GB) lohnenswert, für Mars (<5%) nicht.** |
...
| 40 | ☐ | MoE Load Balancing Expert Frequency | Styx, Hydra | 2-3 Wochen | nein (Konzept) | — | bessere Load-Balance bei MoE-Offloading — **Eval 2026-07-13: Nicht im Fork. Forschungsgestützt (LightMoE ACL 2026, FineMoE, LayerScope). Fork hat nur statisches --n-cpu-moe, keine dynamische Frequency-Tracking. ~1 Woche Aufwand. Ergänzt thecodacus Pinning mit datengetriebener Expert-Platzierung.** |
