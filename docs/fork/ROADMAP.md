...
| 37 | ⏭️ | LFRU Expert Caching | Styx, Hydra | 2-4 Wochen | vLLM commit 71ed1fc | — | +5.2% speedup, 15-20% hit rate — **Eval 2026-07-13: Teilweise implementiert (Tensor-Level Upload Skipping). Root Cause gefunden: thecodacus Prefetch hat NIE auf Styx funktioniert. Expert-Weights haben COMPUTE Buffer-Usage (2) statt WEIGHTS (1), was den op_offload-Mechanism verhindert — MUL_MAT_ID wird auf CPU ausgeführt, kein GPU-Upload nötig. Mit -ncmoe 20 sind beide (Weights + Computation) auf CPU. Selektiver Kopierpfad auch broken: erste Node im Split ist SCALE (32) nicht MUL_MAT_ID (30). Fix versucht: WEIGHTS-Check relaxt zu !=ANY, MUL_MAT_ID-Suche im Split-Graph — aber ohne op_offload gibt es keinen Cross-Backend-Copy. Tensor-Level Cache im generischen Kopierpfad implementiert aber 0 Expert-Weights dort (alle Inputs sind kleine Compute-Tensoren). Root Cause ist vermutlich im tensor_buft_overrides-Buffer-Allokation-Pfad. Postponed bis Buffer-Usage-Fix oder andere Offload-Strategie.** |
