...
| 19 | ✅ | Expected Attention KV Cache Compression | Alle | 4-6 Wochen | arXiv:2510.00636 | Phase 3 ✅ | ergänzt TurboQuant — **Phase 1 ✅: CPU Math (12 Tests, 135 Assertions). Phase 2a/b ✅: CLI-Parameter + cparams. Phase 2c ✅: ea_compress_stub. Phase 2d ✅: Post-hoc Pruning (oldest-first MVP). Phase 3 ✅: Intelligent EA Scoring — Q-Capture via ggml_set_output, Rolling Buffer, Score-basiertes Pruning (E(A)=exp(K@μ'/√d)), NeoX+Interleaved RoPE, V-norm Rescaling, TurboQuant WHT-Forward-Integration (WHT(mean(q))=mean(WHT(q))). 358 Unit-Tests, 14 Code-Review Issues behoben (3 Runden review-swe). Smoke-Test: Gemma 4 12B + turbo3 + EA → korrekt. GPU-Tests auf Styx ausstehend. Bekannte Limitationen: Mean-only (Covariance Phase 4), V-norm nur bei Flash Attention, RoPE-Parameter unvollständig, Buffer-Init mit Layer-0 Dimensionen.** |
