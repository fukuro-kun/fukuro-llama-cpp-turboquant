...
| 57 | ⏭️ | MMQ Stream-k Disable für Tensor-Split MoE | Uranus | 2-4h | PR #22170 (merged) | — | 30-50% P2P PP auf Dual-GPU MoE — **Implementiert 2026-07-13: GGML_CUDA_DISABLE_MMQ_STREAM_K env var hinzugefügt (stream-k disable Teil von PR #22170, ohne cp.async). Kompiliert korrekt auf Hydra + Uranus. Kann nicht benchmarken: Uranus GPUs belegt (15.2/15.8 GB), kein MoE-Modell auf Uranus. Styx (Pascal CC 6.1) hat stream-k ohnehin nicht aktiv (braucht >=Volta). Pending: Benchmark mit MoE-Modell auf Uranus wenn GPUs frei.** |
