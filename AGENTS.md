...
- GPU-Architektur-Build-Matrix (generisch):
  ...
  - **AMD iGPU/APU:** `-DGGML_VULKAN=ON`, ROCm experimentell
    - ✅ **turbo3/4 KV-Cache auf Vulkan FUNKTIONIERT (seit 2026-08-15):** TurboQuant Vulkan-Shader (Commit `cb3b1d571`) sind aktiv. turbo3/4 ohne mmproj ist der aktuelle Production-Standard auf Phobos (TG 25.8 t/s solo, 16.5 t/s parallel). turbo3/3 + mmproj war der vorherige Standard. Historische Benchmarks (`docs/fork/2026-07-09_VULKAN_KV_CACHE_BENCHMARK.md`) sind wieder gültig.
...
|--------|---------------------|-------------------|---------------|-------------|
| Mars (AMD APU, LXC phobos) | ~~180k~~ (obsolet) | **256k (lädt)** | **262144 (256k), 2×128k Slots, turbo3/4 KV, kein mmproj** | turbo4 V + mmproj + 262k: RADV kompiliert turbo4 V FA-Shader extrem langsam (~14 Min/Variante), Cache ineffective. turbo3/4 ohne mmproj funktioniert (seit 2026-08-15 Production). KV-Buffer nur ~1 GB (GQA). Siehe `scripts/start-mars-26b-server.sh` |
