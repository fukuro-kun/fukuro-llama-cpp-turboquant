...
|--------|---------------------|-------------------|---------------|-------------|
| Mars (AMD APU, LXC phobos) | ~~180k~~ (obsolet) | **256k (lädt)** | **262144 (256k), 2×128k Slots, turbo3/3 KV** | turbo4 V + mmproj + 262k: RADV kompiliert turbo4 V FA-Shader extrem langsam (~14 Min/Variante), Cache ineffective. turbo3/3 funktioniert. KV-Buffer nur ~1 GB (GQA). Siehe `scripts/start-mars-26b-server.sh` |
