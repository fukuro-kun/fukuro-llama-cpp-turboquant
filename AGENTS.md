...
| **E4B+MTP auf 8GB Single-GPU** | ❌ **Offen** | E4B's PLE-Architektur (variable V-Embedding-Sizes pro Layer) bricht FA (`fattn.cu:110`), turbo KV-Cache benötigt FA, ohne turbo → OOM. Partieller CPU-Offload (`-ngl <max`) → `GGML_ASSERT` Shape-Mismatch bei Layer-Sharing. Workaround: Ollama (kein turbo, kein custom FA). Siehe `docs/fork/2026-07-11_E4B_MTP_8GB_CRASH.md`. |
